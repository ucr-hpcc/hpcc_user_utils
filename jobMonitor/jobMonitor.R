...
importUserDB <- function(mycol=1:7) {
	userdb <- read.delim("/opt/linux/centos/7.x/x86_64/pkgs/iigb_utilities/1/qstatMonitor/user_details.txt")[ , mycol]
...
nodesUp <- function() {	
	allnodenames <- as.character(read.delim("/opt/linux/centos/7.x/x86_64/pkgs/iigb_utilities/1/qstatMonitor/cluster.conf")[,"Node_ID"])
...
statsReport <- function(refresh=TRUE, plot="png", stdout=TRUE, tmpfile='', ...) {
	...
	userdb <- importUserDB()
	## User run stats
	cluster_conf <- read.delim("/opt/linux/centos/7.x/x86_64/pkgs/iigb_utilities/1/qstatMonitor/cluster.conf")
    ...
    configGraph <- function (margins=TRUE){
        ...
        mtext(paste("Activity Report Generated by qstatMonitor on", format(Sys.time(), "%a, %b %d, %Y, at %H:%M:%S")), side = 3, line = -2, outer = TRUE, cex=1.3, font=2)

        if (margins == TRUE) { dev.off() }
    }

	if(plot=="pdf") {
        tmpfile <- paste("/tmp/qstatMonitor", paste(sample(0:9, 6), collapse=""), ".pdf", sep="")
    ...
    if(plot=="png") {
        png("/opt/linux/centos/7.x/x86_64/pkgs/iigb_utilities/1/qstatMonitor/jobMonitor.png", height=1200, width=1100, pointsize=14)
        configGraph()
