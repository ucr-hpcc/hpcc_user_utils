...
for LIMIT in $(sacctmgr show account $SGROUP format=partition,grptres%30,user --ass --noheader | grep -P "$USER\s*$|gpu\=" | sed "s/\s*$USER\s*//g" | grep -v '^\s*gpu\s*$' | sort | uniq); do
