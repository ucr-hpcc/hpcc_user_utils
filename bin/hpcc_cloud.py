...
def SetComputeInstanceType(computeinstancetype):
    ...
    print("Cluster node type set to {} - {} (@ ${} per hour per node while cluster is running)".format(sys.argv[2], computeinstancetype, GetNodeTypeCost(computeinstancetype)))
    exit()
...
def ReportComputeInstanceType():
    ...
    print("Current node type is {} (@ ${} per hour per node while cluster is running)".format(compute_instance_type, GetNodeTypeCost(compute_instance_type)))
    exit()
...
#                SetComputeInstanceType('p2.8xlarge')
            ...
            if sys.argv[2] == "c5.9xlarge":
                SetComputeInstanceType('c5.9xlarge')
            if sys.argv[2] == "x1.16xlarge":
                SetComputeInstanceType('x1.16xlarge')
            if sys.argv[2] == "i3.2xlarge":
                SetComputeInstanceType('i3.2xlarge')
            if sys.argv[2] == "m5.large":
                SetComputeInstanceType('m5.large')
            if sys.argv[2] == "avail":
                ReportAvailInstanceTypes('t2.micro','test')
                ReportAvailInstanceTypes('c5.2xlarge','base')
#                ReportAvailInstanceTypes('p2.8xlarge','gpu')
                ReportAvailInstanceTypes('r4.large','highmem')
                ReportAvailInstanceTypes('c5.9xlarge','c5.9xlarge')
#                ReportAvailInstanceTypes('x1.16xlarge','x1.16xlarge')
                ReportAvailInstanceTypes('i3.2xlarge','i3.2xlarge')
                ReportAvailInstanceTypes('m5.large','m5.large')
		exit()
        ReportComputeInstanceType()

def ReportAvailInstanceTypes(nodetype, keyword):
    print("Node type: {0:13s} Keyword: {1:13s} Cost: {2:2.3f}".format(nodetype, keyword, GetNodeTypeCost(nodetype,)))

def GetNodeTypeCost(nodetype):
    AWSPricingCache = '/opt/linux/centos/7.x/x86_64/pkgs/iigb_utilities/1/bin/aws-price-cache-file'
    #if not os.path.exists(AWSPricingCache):
        #response = requests.get('https://pricing.us-east-1.amazonaws.com/offers/v1.0/aws/AmazonEC2/current/us-west-2/index.json')
    #    offer = json.loads(response.text)
    #    with open(AWSPricingCache, 'w') as f:
    #        f.write(json.dumps(offer))
    with open(AWSPricingCache) as f:
         offer = json.load(f)
    filtered = []
    terms = offer['terms']
    ...
    for a in filtered:
        ...
        if a['instanceType'] == nodetype:
            return cost

def GetNodeTypeCostTest(nodetype):
    #AWSPricingCache = '/opt/linux/centos/7.x/x86_64/pkgs/iigb_utilities/1/bin/aws-price-cache-file'
    AWSPricingCache = 'linux-od.min.js'
    #if not os.path.exists(AWSPricingCache):
    #response = requests.get('https://pricing.us-east-1.amazonaws.com/offers/v1.0/aws/AmazonEC2/current/us-west-2/index.json')
    #response = requests.get('http://a0.awsstatic.com/pricing/1/ec2/linux-od.min.js')
    #offer = json.loads(response.text)
    #    with open(AWSPricingCache, 'w') as f:
    #        f.write(json.dumps(offer))
    with open(AWSPricingCache) as f:
...
def createcfnconfig():
    if sys.argv[1] == "configure":
        ...
        Config.set('cluster default', 'custom_ami', 'ami-827362e2')
